...
export function GanttChart({ milestones, onUpdateStatus }: GanttChartProps) {
  ...
  const validMilestones = useMemo(() => {
    return milestones.filter((m) => {
      ...
      return !Number.isNaN(new Date(m.dueDate).getTime())
    })
  }, [milestones])
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            No valid milestone dates found yet.
          </p>
        </CardContent>
      </Card>
    )
  ...
  const dates = validMilestones.map((m) => new Date(m.dueDate))
  ...
  const totalDays = rawTotalDays <= 0 ? 1 : rawTotalDays
                  ...
                  <div className="border-t border-border px-3 py-3">
                    ...
                    {milestone.isCriticalPath && (
                      <div className="mb-3 flex items-center gap-2 rounded-lg bg-warning/10 px-3 py-2 text-sm text-warning">
                        ...
                        <span>
                          This is a university deadline and cannot be moved.
                        </span>
                      </div>
                    )}
